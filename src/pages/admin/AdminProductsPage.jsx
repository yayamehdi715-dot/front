      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
          {filtered.map((product) => {
            const totalStock = product.stock ?? product.sizes?.reduce((s, x) => s + x.stock, 0) ?? 0
            return (
              <div key={product._id} className="admin-card group relative overflow-hidden">
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="text-right">
                    <span className={`text-xs font-body block ${totalStock === 0 ? 'text-red-400' : totalStock <= 5 ? 'text-yellow-400' : 'text-brand-gray-500'}`}>
                      {totalStock === 0 ? 'Épuisé' : `${totalStock} en stock`}
                    </span>
                    {(product.purchaseCount ?? 0) > 0 && (
                      <span className="text-xs text-teal-400 font-body block mt-0.5">
                        🔥 {product.purchaseCount} commandé{product.purchaseCount > 1 ? 's' : ''}
                      </span>
                    )}
                  </div>
                </div>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 overflow-y-auto">
          <div className="min-h-screen flex items-start justify-center p-4 pt-10">
            ...
            />
            <div className="relative z-10 bg-brand-gray-900 border border-brand-gray-700
